...
const UpdateCommission = ({ setIsUpdatePop, details }) => {
    const router = useRouter();

    const [loading, setLoading] = useState(false);
    const [userVal, setUserVal] = useState({
        commissionName: details.commissionName,
    ...
    const submitHandler = async () => {
        ...
        try {
            ...
            if (res.ok) {
                ...
                setLoading(false);
                throw new Error("Faild to update commission !");
            }
            ...
            <div className="app-modal-weapper">
                <div className="app-modal-inner modal-animation">
                    <div className="modal-title">
                        <h3>EDIT COMMISSION | ACCOUNT LEVEL</h3>
                        <span><i className="fa fa-times" onClick={() => setIsUpdatePop(false)}></i></span>
                    </div>
                    <form className="global-form" onSubmit={(e) => e.preventDefault()}>
                        ...
                        <div className="global-submit-btn">
                            <button onClick={() => submitHandler()}> {loading ? <i className="fa fa-spinner g-loader"></i> : "UPDATE"} </button>
                        </div>
                    </form>
                </div>
            </div>
