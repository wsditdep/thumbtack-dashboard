...
const Notice = ({ notice, setIsNotice }) => {
    ...
    const submitHandler = async () => {
        try {
            ...
            if (res.ok) {
                ...
                setLoading(false);
                throw new Error("Faild to save notice!");
            }
    ...
    return (
        <>
            <div className="suboperation-pop-wrapper" onClick={() => setIsNotice(false)}>
                <div className="suboperation-pop-childs" onClick={(e) => e.stopPropagation()}>
                    <h3>MANAGE NOTICE</h3>
                    <p>This is the running message that user will see in the platform</p>
                    {
                        isNew
                            ?
